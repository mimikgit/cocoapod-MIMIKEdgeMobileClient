...
///   </li>
/// </ul>
SWIFT_CLASS("_TtC21MIMIKEdgeMobileClient15MIMIKAuthStatus")
@interface MIMIKAuthStatus : NSObject <NSCoding>
- (void)encodeWithCoder:(NSCoder * _Nonnull)aCoder;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder;
...
/// Platform startup with a completion block.
/// important:
...
/// Stops edge with a completion block.
/// important:
...
/// Starts an authentication session in a view controller with a completion block containing the authorization state.
/// note:
...
/// Starts an authentication session in a view controller with a completion block containing the authorization state. Platform content will be erased.
/// warning:
...
/// important:
/// This method has to be called BEFORE calling startEdge (which initalizes mimik edge with a default configuration, if custom one is not specified).
/// \param customConfiguration Custom configuration value.
///
- (void)setCustomConfigurationWithConfiguration:(NSString * _Nonnull)configuration completion:(void (^ _Nonnull)(BOOL))completion;
